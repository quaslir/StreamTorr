...
bool Demuxer::open(const std::string &filename) {
    ...
    int video_index =
        av_find_best_stream(format_context.get(), AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
    ...
    int audio_index =
        av_find_best_stream(format_context.get(), AVMEDIA_TYPE_AUDIO, -1, -1, nullptr, 0);
...
bool Demuxer::open_with_io_context(AVIOContext *io_context) {
    ...
    if (avformat_open_input(&raw_ctx, "", nullptr, nullptr) < 0)
        return false;
    format_context.reset(raw_ctx);
    int video_index =
        av_find_best_stream(format_context.get(), AVMEDIA_TYPE_VIDEO, -1, -1, nullptr, 0);
...
bool Demuxer::is_open() const { return open_; }
...
double Demuxer::duration_seconds() const {
    ...
    if (format_context->duration != AV_NOPTS_VALUE)  return static_cast<double>(format_context->duration) / AV_TIME_BASE;
