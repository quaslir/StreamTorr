...
DecoderSendResult VideoDecoder::send_packet(const AVPacket *packet) {
    int result = avcodec_send_packet(codec_context.get(), packet);

    if (result == 0)
        return DecoderSendResult::Ok;
    else if (result == AVERROR(EAGAIN))
        return DecoderSendResult::NeedsMoreOutput;

    return DecoderSendResult::Error;
}

std::optional<smart_frame> VideoDecoder::receive_frame() {
...
void VideoDecoder::flush() { avcodec_flush_buffers(codec_context.get()); }

const AVCodecContext *VideoDecoder::get_codec_context() const { return codec_context.get(); }
