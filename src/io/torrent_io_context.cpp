...
bool TorrentIOContext::open(TorrentClient *client, const std::filesystem::path &file_path,
    ...
    avio_context_ = avio_alloc_context(avio_buffer_, kBufferSize, 0, this,
                                       &TorrentIOContext::read_packet_callback, nullptr,
                                       &TorrentIOContext::seek_callback);
    if (!avio_context_) {
...
void TorrentIOContext::set_progress_callback(ProgressCallback cb) { progress_cb_ = cb; }

void TorrentIOContext::set_reporting(bool on) { reporting_.store(on, std::memory_order_relaxed); }
...
int TorrentIOContext::read_packet(uint8_t *buf, int buf_size) {
    ...
    if (last_prioritized_pos_ < 0 ||
        std::abs(current_position_ - last_prioritized_pos_) > kRepriorityStep) {
    ...
    if (reporting_.load(std::memory_order_relaxed) && progress_cb_) {
        float current =
            client_->window_progress(static_cast<uint64_t>(absolute_offset), kProbeWindow);
        if (current > max_reported_progress)
            max_reported_progress = current;
        progress_cb_({TorrentStage::OpeningStream, max_reported_progress});
    }
    if (!client_->wait_for_range(static_cast<uint64_t>(absolute_offset),
                                 static_cast<uint64_t>(to_read), kTimeoutMs))
        return AVERROR(EIO);
    file_stream_.clear();
...
AVIOContext *TorrentIOContext::avio_context() const { return avio_context_; }
