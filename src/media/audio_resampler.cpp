...
bool AudioResampler::open(const AVCodecContext *decoder_ctx) {
    ...
    if (swr_alloc_set_opts2(&raw_swr, &out_ch_layout_, out_sample_fmt_, out_sample_rate_,
                            &decoder_ctx->ch_layout, decoder_ctx->sample_fmt,
                            decoder_ctx->sample_rate, 0, nullptr) < 0)
        return false;
    swr_.reset(raw_swr);
...
std::optional<smart_frame> AudioResampler::convert(const AVFrame *frame) {
    ...
    if (av_frame_get_buffer(new_frame.get(), 0) < 0)
        return std::nullopt;
    int converted = swr_convert(swr_.get(), new_frame->data, new_frame->nb_samples,
