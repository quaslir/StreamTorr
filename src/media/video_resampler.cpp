...
bool VideoResampler::open(int w, int h, AVPixelFormat src_format) {
    width_ = w;
    height_ = h;
    sws_.reset(sws_getContext(w, h, src_format, w, h, AV_PIX_FMT_YUV420P, SWS_FAST_BILINEAR,
...
std::optional<smart_frame> VideoResampler::convert(const AVFrame *frame) {
    ...
    if (av_frame_get_buffer(out.get(), 0) < 0)
        return std::nullopt;

    int result =
        sws_scale(sws_.get(), frame->data, frame->linesize, 0, height_, out->data, out->linesize);
    if (result <= 0)
        return std::nullopt;
