...
bool VideoRenderer::resize_texture(int width, int height) {
    ...
    SDL_Texture *texture = SDL_CreateTexture(renderer_.get(), SDL_PIXELFORMAT_IYUV,
                                             SDL_TEXTUREACCESS_STREAMING, width, height);
    if (!texture)
        return false;
    texture_.reset(texture);
...
bool VideoRenderer::update_texture(const AVFrame *frame) {
    ...
    if (frame->width != texture_width_ || frame->height != texture_height_) {
        if (!resize_texture(frame->width, frame->height))
            return false;
    }
    if (!texture_)
        return false;
...
bool VideoRenderer::is_fullscreen() const {
    return (SDL_GetWindowFlags(window_.get()) & SDL_WINDOW_FULLSCREEN_DESKTOP) != 0;
}
