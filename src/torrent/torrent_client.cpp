...
lt::torrent_status TorrentClient::status() const { return handle_.status(); }
...
std::optional<VideoFileInfo> TorrentClient::video_file_info() const {
    ...
    return VideoFileInfo{download_dir_ /
                             files.file_path(static_cast<libtorrent::file_index_t>(video_index)),
                         files.file_offset(static_cast<libtorrent::file_index_t>(video_index)),
                         files.file_size(static_cast<libtorrent::file_index_t>(video_index))};
}
...
bool TorrentClient::is_range_available(uint64_t offset, uint64_t length) const {
    ...
    lt::piece_index_t last_piece{
        static_cast<int>((offset + length - 1) / static_cast<uint64_t>(piece_length))};
    auto status = handle_.status(lt::torrent_handle::query_pieces);
...
bool TorrentClient::wait_for_range(uint64_t offset, uint64_t length, uint32_t timeout_ms) const {
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        if (is_range_available(offset, length))
            return true;
        if (abort_wait_.load(std::memory_order_relaxed))
            return false;
